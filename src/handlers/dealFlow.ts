...
export function registerDealFlow(bot: Telegraf) {
  // Entry point: trigger words start the wizard.
  ...
  bot.action(/^pick_admin_(\d+)$/, async (ctx) => {
    ...
    // Send the admin's payment info.
    if (state.currency === "CRYPTO") {
      if (admin.cryptoAddress) {
        await ctx.reply(`Send payment to this crypto address:\n\n${admin.cryptoAddress}`);
      } else {
      ...
      if (admin.qrFileId) {
        await ctx.replyWithPhoto(admin.qrFileId, { caption: "Scan this QR to pay via UPI." });
      } else {
